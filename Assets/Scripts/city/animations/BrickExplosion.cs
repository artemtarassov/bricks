...
public class BrickExplosion : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        foreach (var brick in bricks)
        {
            ...
            var normalizedTime = Mathf.Clamp01(elapsedTime / brick.flyDuration);
            if (normalizedTime < 1f)
            {
                hasActiveBricks = true;
            }

            var currentTime = Mathf.Min(elapsedTime, brick.flyDuration);
    ...
    [ContextMenu("Play Brick Explosion")]
    public void Play()
    {
        if (bricks.Count == 0)
        {
    ...
    [ContextMenu("Reset Brick Explosion")]
